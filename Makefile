...
qemu-test-%: $(TEST_BUILD_DIR)/%/sapote.iso
	...
	case '$*' in \
		...
		ext4-recovery) \
			grep -Fxq 'ST EXT4 RECOVERY marker cleared transaction committed appended exact truncate revoke rearm create hardlink unlink journal clean transactions 0 replay 0 slots 0 VFS read-only remount clean resources exact' "$$log" && \
