...
| 32. Signed HTTPS package install | `native-phip` | Runs the Ring 3 `phip` client against a deterministic root-signed repository and publisher-signed v3 package, authenticates an exact install plan, downloads every digest-bound payload over TLS, commits generation one to journaled ext4, synchronizes and reboots, independently parses the persisted authority-selected database, runs read-only `e2fsck` on the resulting image, and requires clean process/network/file/upload/controller/service/NVMe censuses plus an encrypted PCAP audit. |
