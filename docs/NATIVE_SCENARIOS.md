...
| 32. Signed HTTPS package lifecycle | `native-phip` | Runs the Ring 3 `phip` client through a deterministic three-boot version-1 install, version-2 update, and signed version-1 downgrade refusal. Both accepted generations download every digest-bound payload over TLS and commit to journaled ext4; each is synchronized and rebooted. The final boot proves generation 2 remained authoritative, launches its installed static application from ext4, verifies exact Data output, runs read-only `e2fsck`, and requires clean process/network/file/upload/controller/service/NVMe censuses plus an encrypted PCAP audit. |
