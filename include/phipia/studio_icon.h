...
#ifndef PHIPIA_STUDIO_ICON_H
#define PHIPIA_STUDIO_ICON_H

#include <stddef.h>
#include <stdint.h>

/* Uses the checked SRL decoder and status values declared by logo.h. */
int32_t phipia_studio_icon_geometry(uint32_t *width, uint32_t *height);
int32_t phipia_studio_icon_decode(
    uint32_t *out,
    size_t out_pixels,
    uint8_t red_shift,
    uint8_t green_shift,
...
int32_t phipia_studio_icon_decode_alpha(uint8_t *out, size_t out_pixels);
