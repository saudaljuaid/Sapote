...
#define SAPOTE_PACKAGE_CONTROL_PLAN_MAX UINT32_C(8)
#define SAPOTE_PACKAGE_CONTROL_TEXT_BYTES UINT32_C(128)
#define SAPOTE_PACKAGE_CONTROL_PATH_BYTES UINT32_C(256)

#define SAPOTE_PACKAGE_CONTROL_PREPARED UINT32_C(1)
#define SAPOTE_PACKAGE_CONTROL_COMMITTED UINT32_C(2)
...
struct sapote_package_control_open_request {
    uint32_t size;
    uint32_t version;
...
struct sapote_package_control_item_request {
    uint32_t size;
    uint32_t version;
    ...
    char package_version[SAPOTE_PACKAGE_CONTROL_TEXT_BYTES];
    char download_path[SAPOTE_PACKAGE_CONTROL_PATH_BYTES];
} __attribute__((packed));

struct sapote_package_control_attach_request {
    uint32_t size;
    uint32_t version;
    sapote_handle_t control;
    uint32_t index;
...
struct sapote_package_control_commit_request {
    uint32_t size;
    uint32_t version;
    sapote_handle_t control;
    uint32_t flags;
...
_Static_assert(sizeof(struct sapote_package_control_open_request) == 56U,
    "Sapote package-control open ABI changed");
_Static_assert(sizeof(struct sapote_package_control_item_request) == 592U,
    "Sapote package-control item ABI changed");
_Static_assert(sizeof(struct sapote_package_control_attach_request) == 40U,
    "Sapote package-control attach ABI changed");
_Static_assert(sizeof(struct sapote_package_control_commit_request) == 48U,
    "Sapote package-control commit ABI changed");
