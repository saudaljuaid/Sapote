#!/usr/bin/env python3
"""Canonical Sapote installed-state and transaction-recovery reference model.
...
PACKAGE_TOOL_PATH = Path(__file__).with_name("sapote-package.py")
PACKAGE_TOOL_SPEC = importlib.util.spec_from_file_location(
    "sapote_transaction_package_tool", PACKAGE_TOOL_PATH)
if PACKAGE_TOOL_SPEC is None or PACKAGE_TOOL_SPEC.loader is None:
    raise RuntimeError("could not load Sapote package tool")
...
AUTHORITY_MAGIC = b"SAPGEN01"
AUTHORITY_BYTES = 128

JOURNAL_MAGIC = b"SAPTXN01"
JOURNAL_BYTES = 512
...
def main() -> int:
    ...
    try:
        ...
        print(f"Sapote transaction state refused: {error}", file=sys.stderr)
        return 1
    return 0
