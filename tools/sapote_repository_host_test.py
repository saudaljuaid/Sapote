...
SPEC = importlib.util.spec_from_file_location(
    "sapote_repository", ROOT / "tools" / "sapote-repository.py")
...
def main() -> int:
    ...
    packages = [
        package("org.sapote.desktop", "3.0.0", desktop_payload,
                dependencies=[
                    {"identifier": "virtual.renderer",
                     "constraint": ">=1.0.0,<2.0.0"},
                    {"identifier": "org.sapote.libc", "constraint": "^1.0.0"},
                ]),
        package("org.sapote.libc", "0.9.0", libc_old_payload),
        package("org.sapote.render-soft", "2.0.0", renderer_payload,
                provides=[{"identifier": "virtual.renderer", "version": "1.5.0"}]),
        package("org.sapote.libc", "1.0.0", libc_payload),
    ]
    ...
    if not crypto_available:
        ...
        print("Sapote repository host tests passed: Ed25519 unavailable and refused closed")
        return 0
    ...
    assert report["repository"] == "org.sapote.main"
    ...
    assert [(item["identifier"], item["version"]) for item in report["packages"]] == [
        ("org.sapote.desktop", "3.0.0"),
        ("org.sapote.libc", "0.9.0"),
        ("org.sapote.libc", "1.0.0"),
        ("org.sapote.render-soft", "2.0.0"),
    ]
    ...
    plan, lock = REPOSITORY.resolve_repository(
        report, ["org.sapote.desktop@^3.0.0"], architecture="x86_64",
    ...
    REPOSITORY.verify_download(
        next(item for item in report["packages"]
             if item["identifier"] == "org.sapote.libc" and item["version"] == "1.0.0"),
        libc_payload)
    libc_entry = next(item for item in report["packages"]
                      if item["identifier"] == "org.sapote.libc"
    ...
    expect_refusal(
        lambda: REPOSITORY.resolve_repository(
            report, ["org.sapote.desktop"], architecture="aarch64",
            abi_version=1),
        "architecture")
    expect_refusal(
        lambda: REPOSITORY.resolve_repository(
            report, ["org.sapote.desktop"], architecture="x86_64",
            abi_version=3),
    ...
    unsatisfied_report = parsed_with([
        package("org.sapote.need-missing", "1.0.0", b"missing",
                dependencies=[{"identifier": "org.sapote.absent",
                               "constraint": "*"}]),
    ])
    expect_refusal(
        lambda: REPOSITORY.resolve_repository(
            unsatisfied_report, ["org.sapote.need-missing"],
            architecture="x86_64", abi_version=1),
        "unsatisfied dependency")
    backtracking_report = parsed_with([
        package("org.sapote.app", "1.0.0", b"app one",
                dependencies=[{"identifier": "org.sapote.lib",
                               "constraint": "^1.0.0"}]),
        package("org.sapote.app", "2.0.0", b"app two",
                dependencies=[{"identifier": "org.sapote.lib",
                               "constraint": "^2.0.0"}]),
        package("org.sapote.lib", "1.1.0", b"lib one"),
    ])
    backtracking_plan, _ = REPOSITORY.resolve_repository(
        backtracking_report, ["org.sapote.app"], architecture="x86_64",
        abi_version=1)
    assert [(item["identifier"], item["version"])
            for item in backtracking_plan] == [
                ("org.sapote.lib", "1.1.0"), ("org.sapote.app", "1.0.0")]
    conflict_packages = copy.deepcopy(packages)
    next(item for item in conflict_packages
         if item["identifier"] == "org.sapote.render-soft")["conflicts"] = [
             {"identifier": "org.sapote.desktop", "constraint": "*"}]
    conflict_report = parsed_with(conflict_packages)
    expect_refusal(
        lambda: REPOSITORY.resolve_repository(
            conflict_report, ["org.sapote.desktop"],
            architecture="x86_64", abi_version=1),
        "conflicts with")
    ambiguous_packages = copy.deepcopy(packages)
    ambiguous_packages.append(
        package("org.sapote.render-hw", "1.0.0", b"hardware renderer",
                provides=[{"identifier": "virtual.renderer", "version": "1.4.0"}]))
    ambiguous_report = parsed_with(ambiguous_packages)
    expect_refusal(
        lambda: REPOSITORY.resolve_repository(
            ambiguous_report, ["org.sapote.desktop"],
            architecture="x86_64", abi_version=1),
        "ambiguous providers")
    cycle_report = parsed_with([
        package("org.sapote.a", "1.0.0", b"a",
                dependencies=[{"identifier": "org.sapote.b", "constraint": "*"}]),
        package("org.sapote.b", "1.0.0", b"b",
                dependencies=[{"identifier": "org.sapote.a", "constraint": "*"}]),
    ])
    expect_refusal(
        lambda: REPOSITORY.resolve_repository(
            cycle_report, ["org.sapote.a"], architecture="x86_64", abi_version=1),
        "dependency cycle")
    chain_report = parsed_with([
        package("org.sapote.a", "1.0.0", b"a",
                dependencies=[{"identifier": "org.sapote.b", "constraint": "*"}]),
        package("org.sapote.b", "1.0.0", b"b",
                dependencies=[{"identifier": "org.sapote.c", "constraint": "*"}]),
        package("org.sapote.c", "1.0.0", b"c"),
    ])
    expect_refusal(
        lambda: REPOSITORY.resolve_repository(
            chain_report, ["org.sapote.a"], architecture="x86_64",
            abi_version=1, max_packages=2),
        "graph exceeds its 2-package bound")

    print("Sapote repository host tests passed: canonical signature, freshness, digests, resolver refusals, exact lock")
    return 0
