...
//! it was. [`sapstudio_io::save::save`] does that by encoding into memory,
...
use sapstudio_io::sprw::{Sound, Spoken, Spool, TRAILER_BYTES, Winder, sound_bounds};
use sapstudio_io::status::IoStatus;
use sapstudio_io::vtt::Spotter;

use crate::SlateStatus;
use sapstudio_model::caption::{Caption, Transcript, captions_over};
...
/// see [`sapstudio_io::sprw`] for why they are not interleaved. So the span is
    ...
    let written = {
        ...
        if let Some(dub) = dub.as_mut() {
            for index in 0..count {
                ...
                let one = TimeRange::new(from, sapstudio_core::Duration::new(1, timebase)?)
...
/// produce and is *not* in general `⌊n·r⌋`. See [`sapstudio_io::sprw`]: which
