...
///
/// The same number `sapstudio_model::MAX_NESTING_DEPTH` refuses to build past,
...
/// [`sapstudio_render::Node::Associate`] is the name of that step, and the
...
pub fn plan(
    ...
    levels
        .try_reserve(1)
        .map_err(|_| SlateStatus::Render(sapstudio_render::RenderStatus::OutOfMemory))?;
    levels.push(Level {
        layers: project
            .sequence(sequence)?
    ...
    loop {
        ...
        let Some(level) = levels.last_mut() else {
            ...
            return Err(SlateStatus::Render(
                sapstudio_render::RenderStatus::UnknownNode,
            ));
        };
...
fn direct_source(
    ...
    project: &Project,
    library: &mut dyn Library,
    asset: &sapstudio_model::MediaAsset,
    layer: &Layer,
...
/// [`sapstudio_model::caption::Reading`]. It also makes this function pure in
...
pub fn burn(
    graph: &mut Graph,
    root: NodeId,
    reading: &sapstudio_model::caption::Reading,
    instant: Instant,
    description: FrameDescription,
) -> Result<NodeId, SlateStatus> {
    ...
    if shown.len() > MAX_LINES_ON_SCREEN {
        return Err(sapstudio_model::ModelStatus::TooManyCaptions.into());
    }
    ...
    let card = graph.add(Node::Type {
        description,
        lines,
        size: sapstudio_core::Rational::new(CAPTION_SIZE.0, CAPTION_SIZE.1)
            .map_err(sapstudio_model::ModelStatus::Time)?,
        across: sapstudio_core::Rational::new(CAPTION_PLACE.0.0, CAPTION_PLACE.0.1)
            .map_err(sapstudio_model::ModelStatus::Time)?,
        down: sapstudio_core::Rational::new(CAPTION_PLACE.1.0, CAPTION_PLACE.1.1)
            .map_err(sapstudio_model::ModelStatus::Time)?,
...
impl Scan {
    ...
    /// # Errors
    ///
    /// As [`plan`], plus [`sapstudio_render::RenderStatus::NotRowLocal`] for a
    /// programme holding something whose rows are not independent — a clip
    /// with a framing on it, or a subsampled description — and
    /// [`sapstudio_render::RenderStatus::NoRowForm`] for one holding a
    ...
    /// # Errors
    ///
    /// [`sapstudio_render::RenderStatus::NotRowLocal`] for a description that
    ...
    pub fn row_description(&self) -> Result<FrameDescription, SlateStatus> {
        Ok(sapstudio_render::row_description(self.description, 0)?)
    }
    ...
    /// # Errors
    ///
    /// [`sapstudio_render::RenderStatus::OutsideDomain`] for a row past the
    /// bottom of the picture, [`sapstudio_render::RenderStatus::NoRowForm`] if
