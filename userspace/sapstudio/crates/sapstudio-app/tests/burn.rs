...
use sapstudio_app::{SlateStatus, timeline};
use sapstudio_core::{Digest, Duration, Instant, Timebase};
use sapstudio_media::colour::{AlphaState, ColourDescription};
use sapstudio_media::{Frame, FrameDescription, FramePool, Geometry, PixelFormat};
use sapstudio_model::caption::{self, Caption};
...
fn reading(project: &Project, sequence: SequenceId, instant: Instant) -> caption::Reading {
    let one = sapstudio_core::TimeRange::new(
        instant,
        sapstudio_core::Duration::new(1, instant.timebase()).expect("a duration"),
    )
    .expect("a span");
...
fn two_speakers_at_once_are_two_lines_of_one_block() {
    ...
    let sapstudio_render::Node::Over { layers } = graph.node(top).expect("a node") else {
        panic!("the burn did not composite")
    };
    let sapstudio_render::Node::Type { lines, .. } = graph.node(layers[1]).expect("a node") else {
...
fn more_lines_than_fit_on_screen_are_refused() {
    ...
    assert_eq!(
        ...
        Some(SlateStatus::Model(
            sapstudio_model::ModelStatus::TooManyCaptions
        ))
    );
...
impl caption::Transcript for Counted<'_> {
    fn captions(
        ...
        from: i64,
        to: i64,
    ) -> Result<std::vec::Vec<Caption>, sapstudio_model::ModelStatus> {
        self.asked += 1;
...
fn a_span_is_read_once_and_burned_frame_by_frame() {
    ...
    let over = sapstudio_core::TimeRange::new(at(0), frames(20)).expect("a span");
    let reading =
...
fn a_stretch_holds_the_same_card_for_all_of_its_frames() {
    ...
    let mut held = caption::Held::new(&project);
    let over = sapstudio_core::TimeRange::new(at(0), frames(12)).expect("a span");
