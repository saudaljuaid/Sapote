...
use sapstudio_app::export::{self, Dub, Job, WINDOW_BYTES};
...
use sapstudio_io::sprw::{self, Spool, TRAILER_BYTES};
...
fn a_sidecar_says_what_the_reel_says() {
    ...
    let asset = MediaAsset::new(Digest::of(b"an interview"), RATE, frames(1000))
        ...
        .with_captions(std::vec![
            ...
            sapstudio_model::caption::Caption::new(10, 20, 1, "the second thing")
                .expect("a caption"),
        ])
        .expect("a transcript");
    ...
    let mut storage = MemoryStorage::new(1 << 20);
    let mut held = sapstudio_model::caption::Held::new(&project);
    export::export(&job, &mut library, None, Some(&mut held), &mut storage).expect("an export");
    let reel = sprw::decode(storage.stored().expect("a reel")).expect("a reel");

    let mut sidecar = std::vec::Vec::new();
    let mut spotting = sapstudio_model::caption::Held::new(&project);
...
fn an_exported_reel_carries_the_words_the_programme_showed() {
    ...
    let asset = MediaAsset::new(Digest::of(b"an interview"), RATE, frames(1000))
        ...
        .with_captions(std::vec![
            sapstudio_model::caption::Caption::new(0, 10, 0, "the first thing").expect("a caption"),
            sapstudio_model::caption::Caption::new(10, 20, 0, "the second thing")
                .expect("a caption"),
        ])
        .expect("a transcript");
    let media = project.add_media(asset).expect("room");
