...
//! Sapote's filesystem contract, in types SapStudio can hold.
//!
//! Every constant and every rule in this module is read out of Sapote rather
//! than guessed at, and the source is named beside each one. It is written
//! against **Sapote 2.1.0** at commit `8fe1817`; when Sapote moves, this is
...
//! ## The rule, in full
//!
//! Sapote deliberately accepts a case-insensitive ASCII 8.3 subset:
...
/// How many components a path may have. `SAPFS_MAX_DEPTH`.
...
/// How large one file may be. `SAPFS_MAX_FILE_BYTES`, sixteen mebibytes.
///
...
/// How many live entries one directory may hold. `SAPFS_MAX_LIST_ENTRIES`.
...
/// How many open handles the kernel owns at once. `SAPFS_MAX_HANDLES`.
///
...
/// The punctuation Sapote's 8.3 subset accepts, beside letters and digits.
///
...
/// One 8.3 name, uppercased, as Sapote stores it.
///
...
impl Name {
    ...
    ///
    /// Lowercase is accepted and uppercased, exactly as Sapote does, so the
    ...
    pub fn new(text: &str) -> Result<Self> {
        ...
        for (index, byte) in raw.iter().copied().enumerate() {
            if byte == b'.' {
                // One dot, and neither at the front nor at the back. Sapote
    ...
    /// The name as Sapote would store it.
    #[must_use]
...
/// bytes — each of which Sapote refuses separately, so each is refused
/// separately here.
