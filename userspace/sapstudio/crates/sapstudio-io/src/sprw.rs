...
use sapstudio_model::caption::Caption;
...
impl Sound {
    ...
    /// [`sapstudio_audio::MAX_CHANNELS`], or for more than
    /// [`sapstudio_audio::MAX_SAMPLES`].
    pub fn new(rate: SampleRate, channels: usize, samples: usize) -> Result<Self> {
        if channels == 0 || channels > sapstudio_audio::MAX_CHANNELS {
            return Err(IoStatus::Sound(
                sapstudio_audio::AudioStatus::ChannelCountUnsupported,
            ));
        }
        if samples > sapstudio_audio::MAX_SAMPLES {
...
///
/// The floor of it is what [`sapstudio_core::Instant::floor_into`] gives for
...
impl Reel {
    ...
    pub fn with_captions(mut self, captions: Vec<Caption>) -> Result<Self> {
        ...
        sapstudio_model::caption::checked(&captions).map_err(IoStatus::Model)?;
...
pub const CAPTION_LARGEST: usize = CAPTION_FIXED + sapstudio_model::caption::MAX_CAPTION_TEXT * 4;
...
impl Spool {
    ...
    /// from [`sapstudio_core::Instant::floor_into`] of `k` up to the same of
...
impl Winder {
    ...
    pub fn begin(
        ...
        let line = sapstudio_render::row_description(description, 0).map_err(IoStatus::Render)?;
        let rows =
