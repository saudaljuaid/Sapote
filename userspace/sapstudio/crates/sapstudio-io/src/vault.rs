...
/// [`sapstudio_render::Library`], with nothing in between translating — and
...
pub struct Shelf<'a> {
    vault: &'a Vault,
    looks: &'a [(Digest, sapstudio_render::Look)],
}
...
impl<'a> Shelf<'a> {
    ...
    #[must_use]
    pub const fn new(vault: &'a Vault, looks: &'a [(Digest, sapstudio_render::Look)]) -> Self {
        Self { vault, looks }
    }
}

impl sapstudio_render::Library for Shelf<'_> {
    fn frame(
        &mut self,
        media: Digest,
        tick: i64,
        description: sapstudio_media::FrameDescription,
    ) -> sapstudio_render::Result<sapstudio_media::Frame> {
        let item = self
            .vault
            .get(media)
            .ok_or(sapstudio_render::RenderStatus::MediaAbsent)?;
        let reel = crate::sprw::decode(item.bytes())
            .map_err(|_| sapstudio_render::RenderStatus::MediaUnreadable)?;
        ...
        let at = usize::try_from(tick).map_err(|_| sapstudio_render::RenderStatus::FrameAbsent)?;
        let frame = reel
            .frames()
            .get(at)
            .ok_or(sapstudio_render::RenderStatus::FrameAbsent)?;
        if frame.description() != &description {
            ...
            return Err(sapstudio_render::RenderStatus::SourceDescriptionMismatch);
        }
        Ok(frame.clone())
    }
    ...
    fn look(&mut self, look: Digest) -> sapstudio_render::Result<sapstudio_render::Look> {
        self.looks
            .iter()
            .find(|(digest, _)| *digest == look)
            .map(|(_, held)| held.clone())
            .ok_or(sapstudio_render::RenderStatus::LookAbsent)
    }
}
...
impl sapstudio_model::caption::Transcript for Stacks<'_> {
    ...
    fn captions(
        ...
        from: i64,
        to: i64,
    ) -> sapstudio_model::Result<alloc::vec::Vec<sapstudio_model::caption::Caption>> {
        ...
        spool
            ...
            .map_err(|_| sapstudio_model::ModelStatus::OutOfMemory)
    }
}

impl sapstudio_audio::SampleSource for Stacks<'_> {
    ...
    fn samples(
        ...
        start: i64,
        count: usize,
    ) -> sapstudio_audio::Result<sapstudio_audio::AudioBuffer> {
        let entry = self
            .catalogue
            .find(self.storage, media)
            .map_err(|_| sapstudio_audio::AudioStatus::NotMixable)?
            .ok_or(sapstudio_audio::AudioStatus::NotMixable)?;
        let material = Material::new(self.catalogue, &entry, self.storage);
...
impl sapstudio_render::Library for Stacks<'_> {
    fn frame(
        &mut self,
        media: Digest,
        tick: i64,
        ...
        spool
            .frame(&material, at)
            .map_err(|_| sapstudio_render::RenderStatus::FrameAbsent)
    }
    ...
    fn row(
        ...
        if description.format().plane_count() != 1 {
            ...
            return Err(sapstudio_render::RenderStatus::NoRowForm);
        }
        let at = usize::try_from(tick).map_err(|_| sapstudio_render::RenderStatus::FrameAbsent)?;
        let stride = description
            .format()
            .plane_row_bytes(description.geometry(), 0)
            .map_err(sapstudio_render::RenderStatus::Media)?;
        let mut samples = alloc::vec::Vec::new();
        samples
            .try_reserve(stride)
            .map_err(|_| sapstudio_render::RenderStatus::OutOfMemory)?;
        samples.resize(stride, 0);
        spool
            .plane_row(&material, at, 0, row, &mut samples)
            .map_err(|_| sapstudio_render::RenderStatus::FrameAbsent)?;
        let one = sapstudio_media::FrameDescription::new(
        ...
        .map_err(sapstudio_render::RenderStatus::Media)?;
        let plane = sapstudio_media::Plane::new(samples, stride)
            .map_err(sapstudio_render::RenderStatus::Media)?;
        ...
        planes
            ...
            .map_err(|_| sapstudio_render::RenderStatus::OutOfMemory)?;
        planes.push(plane);
        sapstudio_media::Frame::new(one, planes).map_err(sapstudio_render::RenderStatus::Media)
    }
}
