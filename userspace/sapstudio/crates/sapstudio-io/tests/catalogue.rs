...
fn a_project_and_a_vault_are_committed_separately() {
    ...
    let project = sapstudio_model::Project::new();
    sapstudio_io::save(&project, &mut storage).expect("a project");
    assert_eq!(
    ...
    assert_eq!(sapstudio_io::load(&storage).expect("a load"), project);
}
