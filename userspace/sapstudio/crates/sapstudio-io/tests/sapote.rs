...
//! Sapote's filesystem contract, checked against Sapote's own rules.
//!
//! Every case here is read out of `src/rust/fat32.rs` and `docs/FAT32.md` at
//! Sapote 2.1.0, commit `8fe1817`. A name this accepts is a name Sapote's
//! `parse_component` accepts, and a name this refuses is one Sapote refuses —
...
fn eight_and_three_is_eight_and_three() {
    ...
    // the total accepts it and Sapote does not.
...
#[test]
fn the_punctuation_is_exactly_sapotes() {
    // Sixteen marks, and the test writes them out rather than reading the
    // constant back: `$%'-_@~`!(){}^#&`.
    for mark in "$%'-_@~`!(){}^#&".bytes() {
        assert!(
            is_canonical(mark),
            "Sapote accepts {} and this does not",
            mark as char
        );
    }
    ...
    for mark in " ,;=+[]\\/\"*?<>|:".bytes() {
        assert!(
            !is_canonical(mark),
            "Sapote refuses {} and this does not",
            mark as char
        );
    }
}
...
#[test]
fn the_path_bounds_are_sapotes() {
...
#[test]
fn the_bounds_are_the_ones_sapote_documents() {
    ...
    assert_eq!(MAX_FILE_BYTES, 16_777_216, "SAPFS_MAX_FILE_BYTES");
    assert_eq!(MAX_DIRECTORY_ENTRIES, 64, "SAPFS_MAX_LIST_ENTRIES");
    ...
    // of Sapote's files, with 188,416 bytes to spare; three do not.
    //
