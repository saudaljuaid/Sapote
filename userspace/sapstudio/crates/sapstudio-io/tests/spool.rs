...
use sapstudio_abi::seam::Slot;
use sapstudio_core::{Digest, Rational, Timebase};
use sapstudio_io::sprw::{HEADER_BYTES, MAX_FRAMES, Spool, TRAILER_BYTES};
use sapstudio_io::vault::{Catalogue, Material, Stacks, Vault, store};
use sapstudio_io::{Extent, IoStatus, MemoryStorage, Reel, sprw};
use sapstudio_media::colour::{
    ColourDescription, MatrixCoefficients, Primaries, Range, TransferFunction,
};
use sapstudio_media::{Frame, FrameDescription, Geometry, PixelFormat, Plane};
...
fn described(width: u32, height: u32, format: PixelFormat) -> FrameDescription {
    FrameDescription::new(
        ...
        if format == PixelFormat::Rgba8 {
            Some(sapstudio_media::colour::AlphaState::Premultiplied)
        } else {
            None
        },
        Rational::ONE,
...
fn a_reel_in_a_vault_is_read_without_loading_either() {
    ...
    let looks: std::vec::Vec<(Digest, sapstudio_render::Look)> = std::vec::Vec::new();
    ...
    assert_eq!(
        ...
        Err(sapstudio_render::RenderStatus::FrameAbsent)
    );
    assert_eq!(
        stacks.frame(digest, 0, described(4, 4, PixelFormat::Rgb8)),
        Err(sapstudio_render::RenderStatus::SourceDescriptionMismatch)
    );
    assert_eq!(
        stacks.frame(
        ...
        Err(sapstudio_render::RenderStatus::MediaAbsent)
    );
    assert_eq!(
        storage.whole_reads(),
        before,
...
fn material_that_is_not_a_reel_says_unreadable() {
    ...
    assert_eq!(
        ...
        Err(sapstudio_render::RenderStatus::MediaUnreadable)
    );
}
...
fn a_row_is_rendered_from_storage_without_loading_anything() {
    ...
    use sapstudio_render::{Graph, Node};
    ...
    let whole = graph
        .evaluate(
            faded,
            &mut sapstudio_media::FramePool::new(8, 1 << 16),
            &mut stacks,
        )
...
fn a_row_of_material_that_is_not_there_says_so() {
    use sapstudio_render::{Graph, Library, Node};
...
fn a_vault_serves_sound_without_loading_the_reel() {
    ...
    use sapstudio_audio::SampleSource;
    ...
    assert!(
        stacks
            .samples(sapstudio_core::Digest::of(b"not here"), 0, 10)
            .is_err()
    );
}
...
fn a_vault_serves_the_words_without_loading_the_reel() {
    ...
    use sapstudio_model::caption::Transcript;
    ...
    let take = Reel::new(RATE, frames)
        ...
        .with_captions(std::vec![
            sapstudio_model::caption::Caption::new(0, 10, 0, "so I said to him")
                .expect("a caption"),
            sapstudio_model::caption::Caption::new(10, 20, 1, "and what did he say")
                .expect("a caption"),
        ])
        .expect("a transcript");
    ...
    assert!(
        stacks
            .captions(sapstudio_core::Digest::of(b"not here"), 0, 100)
            .expect("a read")
            .is_empty()
    );
