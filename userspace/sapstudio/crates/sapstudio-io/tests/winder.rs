...
use sapstudio_io::bytes::{Extent, Sink};
use sapstudio_io::memory::{Fault, MemoryStorage};
use sapstudio_io::save::{Scratch, Staged};
use sapstudio_io::sprw::{self, HEADER_BYTES, MAX_FRAMES, Reel, Spool, TRAILER_BYTES, Winder};
use sapstudio_io::status::IoStatus;
use sapstudio_media::colour::{AlphaState, ColourDescription};
use sapstudio_media::{Frame, FrameDescription, Geometry, PixelFormat, Plane, TestPattern};
    ...
    FrameDescription::square(
        ...
        if format.is_rgb() {
            ...
            ColourDescription::new(
                ...
                sapstudio_media::colour::TransferFunction::Bt709,
                sapstudio_media::colour::MatrixCoefficients::Bt709,
                sapstudio_media::colour::Range::Limited,
            )
        },
...
fn wind(sink: &mut dyn Sink, reel: &Reel) -> sapstudio_core::Digest {
    ...
    for rate in [
        ...
        for timebase in [
            ...
            for frames in [0_usize, 1, 2, 3, 29, 30, 1001, 30_000] {
                ...
                let walked =
                    sapstudio_core::Instant::new(i64::try_from(frames).expect("a count"), timebase)
...
fn transcript() -> std::vec::Vec<sapstudio_model::caption::Caption> {
    std::vec![
        sapstudio_model::caption::Caption::new(0, 10, 0, "the first thing").expect("a caption"),
        sapstudio_model::caption::Caption::new(5, 15, 1, "and the answer").expect("a caption"),
        sapstudio_model::caption::Caption::new(10, 30, 0, "then a long one").expect("a caption"),
    ]
}
...
fn a_caption_out_of_turn_or_past_the_length_declared_is_refused() {
    ...
    let long = sapstudio_model::caption::Caption::new(0, 1, 0, &words).expect("a caption");
...
fn a_transcript_is_read_in_windows_and_never_held() {
    ...
    let count = 400_usize;
    let words: std::vec::Vec<sapstudio_model::caption::Caption> = (0..count)
        .map(|index| {
            let from = i64::try_from(index).expect("a tick") * 4;
            let text: std::string::String = core::iter::repeat_n('x', 1 + index % 37).collect();
            sapstudio_model::caption::Caption::new(from, from + 4, 0, &text).expect("a caption")
        })
        .collect();
