...
///
/// The third of these — [`sapstudio_render::Library`] serves pictures and
/// `sapstudio_audio::SampleSource` serves samples — and it arrives for the
...
pub struct Shown {
    ...
    pub from: sapstudio_core::Instant,
    /// The first tick past them.
    pub to: sapstudio_core::Instant,
            ...
            for item in track.items() {
                ...
                if let Some(inner) = asset.nested() {
                    ...
                    for caption in words {
                        ...
                        let here =
                            (
                                offsets.0.checked_add(begins).ok_or(ModelStatus::Time(
                                    sapstudio_core::CoreStatus::Overflow,
                                ))?,
                        ...
                        found.try_reserve(1).map_err(|_| ModelStatus::OutOfMemory)?;
                        found.push(Shown {
                            from: sapstudio_core::Instant::new(from, timebase),
                            to: sapstudio_core::Instant::new(to, timebase),
                            track: index,
                            caption,
                        });
                    }
...
pub struct Reading {
    ...
    timebase: sapstudio_core::Timebase,
}
...
impl Reading {
    ...
    pub fn stretch(&self, instant: sapstudio_core::Instant) -> Result<usize> {
    ...
    pub fn span(&self, stretch: usize) -> Result<sapstudio_core::TimeRange> {
        let from = *self
            .edges
            .get(stretch)
        ...
        let to = *self
            ...
            .get(stretch + 1)
            .ok_or(ModelStatus::OutsideTheReading)?;
        sapstudio_core::TimeRange::new(
            sapstudio_core::Instant::new(from, self.timebase),
            sapstudio_core::Duration::new(to - from, self.timebase).map_err(ModelStatus::Time)?,
        )
...
pub fn captions_at(
    project: &crate::Project,
    sequence: crate::SequenceId,
    instant: sapstudio_core::Instant,
    transcript: &mut dyn Transcript,
) -> Result<alloc::vec::Vec<Shown>> {
    let one = sapstudio_core::TimeRange::new(
        instant,
        sapstudio_core::Duration::new(1, instant.timebase()).map_err(ModelStatus::Time)?,
    )
    .map_err(ModelStatus::Time)?;
