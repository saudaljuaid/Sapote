...
impl Clip {
    ...
    #[must_use]
    pub fn marker_at(&self, at: sapstudio_core::Instant) -> Option<&crate::marker::Marker> {
    ...
    fn noted_from(&self, offset: i64) -> Result<alloc::vec::Vec<crate::marker::Marker>> {
        ...
        for held in &self.markers {
            ...
            moved.push(held.moved_to(sapstudio_core::Instant::new(
                ticks,
                self.duration.timebase(),
            ))?);
        }
    ...
    fn noted_with(&self, later: &Self, by: i64) -> Result<Self> {
        ...
        for held in &later.markers {
            let ticks = held
                ...
                .checked_add(by)
                .ok_or(ModelStatus::Time(sapstudio_core::CoreStatus::Overflow))?;
            joined = joined.with_marker(held.moved_to(sapstudio_core::Instant::new(
                ticks,
                self.duration.timebase(),
            ))?)?;
        }
    ...
    pub fn source_at(&self, offset: i64) -> Result<i64> {
        let travel = match &self.playback {
            ...
            Playback::Ramp(curve) => curve.area_to(sapstudio_core::Instant::new(
                offset,
                self.duration.timebase(),
            ))?,
        };
