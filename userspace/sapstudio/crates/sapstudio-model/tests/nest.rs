...
fn a_nest_has_nowhere_to_be() {
    ...
    assert_eq!(
        project
            .set_media_location(
                asset,
                Some(sapstudio_model::Location::new(b"/somewhere").expect("a hint")),
            )
...
fn a_clip_cuts_from_a_nest_like_any_other_media() {
    ...
    let stack = project
        .sequence(outer)
        .expect("a sequence")
        .stack_at(sapstudio_model::Lane::Picture, at(120))
        .expect("a stack");
...
fn a_nested_clip_carries_everything_a_clip_carries() {
    ...
    project
        .apply(
            ...
            Edit::SetClipMask {
                ...
                index,
                mask: Some(
                    sapstudio_model::Mask::rectangle(
                        sapstudio_core::Rational::new(1, 4).expect("a rational"),
                        sapstudio_core::Rational::new(1, 4).expect("a rational"),
                        sapstudio_core::Rational::new(3, 4).expect("a rational"),
                        sapstudio_core::Rational::new(3, 4).expect("a rational"),
                    )
                    .expect("a mask"),
                ),
    ...
    let stack = project
        .sequence(outer)
        .expect("a sequence")
        .stack_at(sapstudio_model::Lane::Picture, at(102))
        .expect("a stack");
    let layer = &stack[0];
    ...
    assert_eq!(
        layer.fade(),
        sapstudio_core::Rational::new(2, 5).expect("a rational"),
