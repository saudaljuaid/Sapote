...
shows, and it cost `sapstudio-render` nothing at all.

`sapstudio_io::format::encode` is the largest single function at **13,988
...
until `SAP-01` and `SAP-02` exist as a general facility. So the arena's actual
...
because there is no `SAP-03` to load it with.
...
mutation, allocation and teardown owned by C. First Environment's SapStudio
