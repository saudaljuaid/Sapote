...
[`MediaSource`](../crates/sapstudio-model/src/media.rs), beside a recording
and a title.
...
`sapstudio-model` +5,996 (`Project::apply` +2,765 for the refresh-check-undo
path and `refresh_nests` +2,032, which is new), `sapstudio-io` +4,062,
`sapstudio-app` +4,000 (`plan` +2,363 and `decorate` +1,494 for the level
stack), `sapstudio-media` +656 for `Frame::clear`, and `sapstudio-render`
...
And `sapstudio-model` has passed `sapstudio-render` to become the largest crate
...
`without_marker`, `note` and `unnote` most of the rest. `sapstudio-model` is now
...
| A path | 127 bytes, 16 components, relative to one mount | `FAT32_PATH_BYTES`, `SAPFS_MAX_DEPTH` |
| A directory | 64 live entries | `SAPFS_MAX_LIST_ENTRIES` |
| A volume | 256 directories, 129,022 clusters of 512 bytes | `docs/FAT32.md` |
| A file | 16 MiB | `SAPFS_MAX_FILE_BYTES` |
| Open at once | 16 generation-authenticated handles | `SAPFS_MAX_HANDLES` |
...
[`sapstudio_render::Library`] over a vault directly, with nothing in between
...
`Stacks` implements `sapstudio_render::Library` over the whole chain, so the
...
So the trait moved down into `sapstudio-audio` beside the buffer it produces,
...
`sapstudio_render::Library` serves pictures. `sapstudio_audio::SampleSource`
serves samples. `sapstudio_model::caption::Transcript` now serves words, and it
...
turn on.* The number is `SAP-03`'s — twelve image pages, four stack, two heap,
