...
handles. Vendoring alone does not integrate a Sapote shared library or promise
compression support to applications.

The initial x86_64 Sapote build should compile exactly these sources:
...
library and every consumer of `zlib.h`. On Sapote's x86_64 LP64 target, also
...
The two explicit `HAVE_*` values keep Sapote's `-Wundef -Werror` policy while
...
returns `Z_STREAM_ERROR`. Sapote's adapter must check multiplication and size
...
`src/`; carry Sapote adapters outside the upstream tree and refresh provenance
